    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#fbf5e6] to-[#f4ecdc] p-4">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-[#928a76] text-center">{t('radicals_quiz_meanings_title')}</p>
        </div>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex justify-between items-center mb-2">
            <h1 className="text-2xl font-bold text-[#1c1813]">{t('radicals_meanings_of_radicals_title')}</h1>
            <div className="text-[#928a76]">
        ...
        <div className="bg-[#fbf5e6] rounded-xl p-8 border border-[rgba(28,24,19,0.10)] mb-6 text-center">
          <h3 className="text-lg text-[#928a76] mb-4">{t('radicals_meaning_question_header')}</h3>

          {/* Radical objetivo */}
          <div className="text-8xl font-bold text-[#1c1813] mb-4 py-4">
            {currentQ.radical}
          </div>

          {/* Pinyin del radical */}
          {currentQ.pinyin && (
            <div className="text-xl text-[#928a76] mb-6">
              {currentQ.pinyin}
            </div>
          )}

          <h4 className="text-lg text-[#928a76] mb-6">{t('radicals_select_correct_meaning')}</h4>
          ...
          <div className="space-y-3">
            {currentQ.options.map((option) => {
              ...
              let buttonClass = "bg-[#f8f1de] hover:bg-[#bdb39a] border-[rgba(28,24,19,0.18)] text-left";

              if (isAnswered) {
                if (isCorrect) {
                  buttonClass = "bg-[#2f6b4a] border-[#2f6b4a] text-left";
                } else if (isSelected && !isCorrect) {
                  buttonClass = "bg-[#c8392f] border-[#c8392f] text-left";
                } else {
                  buttonClass = "bg-[#f8f1de] border-[rgba(28,24,19,0.18)] opacity-50 text-left";
                }
              }
              ...
              return (
                <button
                  ...
                  disabled={isAnswered}
                  className={`${buttonClass} border-2 rounded-xl p-4 text-lg font-semibold text-[#1c1813] transition disabled:cursor-not-allowed w-full text-center`}
                >
                  {option}
                </button>
              );
