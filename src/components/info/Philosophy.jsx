    ...
    <div className="min-h-screen bg-gray-900 p-4">
      <Container>
        ...
        <div className="bg-gray-800 rounded-2xl shadow-2xl p-6 md:p-8 border border-gray-700">
          <h1 className="text-3xl font-bold text-white mb-2">🧘‍♂️ {t('info_philosophy_thought_title')}</h1>
          <p className="text-gray-300 mb-6">{t('info_philosophy_thought_description')}</p>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {data.philosophy.schools.map((school, i) => (
                <div key={i} className="rounded-2xl border border-gray-600 p-4 hover:shadow-lg transition bg-gray-700">
                  <div className="text-xl font-bold text-white mb-2">{school.name}</div>
                  {school.coreIdeas && (
                  ...
                  {school.figures && (
                    <div className="text-gray-400 text-sm">{t('info_figures_label')} {school.figures.join(", ")}</div>
                  )}
                </div>
              ))}
            </div>
